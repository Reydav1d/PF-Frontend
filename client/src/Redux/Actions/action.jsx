...
export const getSearchAdnFilterProducts = (urlData) => async (dispatch) => {
  try {
    ...
    search ? (search = search.replace(/\s/g, "%20")) : (search = "");

    const getProducts = await axios.get(
      `/filter-sorts/selection?search=${search}&category=${category}&price_min=${price_min}&price_max=${price_max}&sort_by=${sort_by}&order=${order}`
    );
